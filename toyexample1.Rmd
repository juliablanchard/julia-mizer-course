...
cdat<-read.csv("data/time-averaged-catches.csv") ### CHECK UNITS!!! '000 tonnes?

# the fishing moratlty rates are already stored in the param object as params@species_params$catchability

# let's start again and replace with the initial pre-calibration "guessed" Rmax and Kappa
params@resource_params$kappa = 5e10
# penalise the large species with higher density dependence
params@species_params$Rmax <- params@resource_params$kappa*params@species_params$w_inf^-3
# or use erepro
#params@species_params$erepro <- c(rep(1,10),rep(0.01,2))
...
Some more species are coexisting. You can play with the above parameters but but it would take a lot of trial an error to acheive the right combination.
...
We can explore the effects further using Rshiny app, where we also have a plot of the catch data.
...
The shiny app helps with understanding the model but it is tricky to arrive at the bets fit especially if we want to change several species parameter combinations at a time.
Let's choose some values that enable the most species to coexist as a starting point for optimisation. Note we won't vary erepro at the same time as Rmax (they depend on each other). However we will use the value of erepro selcted form the shiny app.

#### Step 3. Calibrate the maximum recruitment, \Rmax, which will affect the relative biomass of each species (and, combined with the fishing parameters, the catches) by minimising the error between observed and estimated catches (again or biomasses). We will also include kappa here.


This will take AWHILE, so go watch some Netflix.

```{r}
# set initial parameter
# kappa, 12 Rmaxs, log10 scale
vary=c(12,rep(6,10))

#the following getError function combines the steps of the optimisastion above - this time with the multispeices model and output the predicted size spectrum

getError <- function(vary,params=initparam,dat=cdat$Catch_8595,data_type="catch",timetorun=100) {
  params@species_params$Rmax[]<-10^vary[2:13]
    params@species_params$erepro[] <-1
  params <- setParams(params,kappa=10^vary[1])
  sim <- project(params, effort = 1, t_max = timetorun, dt=0.1)
          ...
          if (data_type=="catch") {
         output <-getYield(sim)[timetorun,]/1e3 #### CHECK UNITS !! tonnes?
          }
  pred <- log(output)
  dat  <- log(dat)
...
err<-getError(vary,params,dat=cdat$Catch_8595,data_type="catch")
#test
err
...
initparams <- params
# this time carry out optimisation, using optim()
vals<-optim(par=vary,getError,params=initparams,method ="L-BFGS-B",lower=rep(9,13),upper= rep(13,13))



# plug back into model
params@species_params$R_max <- 10^vals$par[2:13] 
params <- setParams(params,kappa=10^vals$par[1])
...
pred_yield <- getYield(sim)[100,]/1e3
obs_yield <- cdat$Catch_8595
plot(log10(pred_yield),log10(obs_yield))
abline(0,1)

# what's going on?

# rerun calibration with these new values as starting values? wrong units??
...
plotDiet(params,"Cod") ##  feeding too much on background resource
plotlyBiomass(sim) ##  feeding too much on background resource
plotlyBiomass(sim) ##  feeding too much on background resource
